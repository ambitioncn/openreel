...
test("DATA-1 SQLite migrations are versioned and legacy JSON import is replay-safe", () => {
  ...
  const db = new DatabaseSync(paths.database, { readOnly: true }); assert.deepEqual(db.prepare("SELECT version FROM schema_migrations").all().map(x => x.version), [1]); assert.equal(db.prepare("SELECT instr(data, ?) AS found FROM entities WHERE kind='assets'").get(fixture.toString("base64")).found, 0); db.close();
});
