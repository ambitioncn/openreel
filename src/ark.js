...
function providerRequest(model, operation, input, taskId) {
  ...
  const firstFrameConditioned = content.some(part => part?.type === "image_url" && part?.role === "first_frame");
  return { url: model.endpoint, method: "POST", body: { model: model.providerModel, content, ...(value.generate_audio === undefined ? {} : { generate_audio: Boolean(value.generate_audio) }), ...(value.resolution ? { resolution: value.resolution } : {}), ...(!firstFrameConditioned && value.ratio ? { ratio: value.ratio } : {}), ...(value.duration ? { duration: value.duration } : {}), ...(value.watermark === undefined ? {} : { watermark: Boolean(value.watermark) }) } };
}
