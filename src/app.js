...
document.querySelectorAll("[data-step-target]").forEach(button => {
  button.onclick = () => {
    ...
    if (!target || target.hidden) {
      $("#short-video-state").textContent = ui("Complete the earlier project steps before opening this section.", "请先完成前面的项目步骤，再打开这一部分。");
      $("#brief-panel").scrollIntoView({ behavior: "smooth", block: "start" });
      return;
    }
    document.querySelectorAll(".flow-steps li").forEach(item => item.classList.toggle("active", item.contains(button)));
...
function renderFinalWorkbench() {
  ...
  $("#workbench-final-state").textContent = preview.asset ? ui("The final MP4 is ready. Download reuses this qualified render; it will not start a local re-render.", "最终 MP4 已就绪。下载会直接使用这份合格渲染，不会启动本地重渲染。") : preview.canExport ? ui("The timeline is ready. Review it before creating a local MP4 preview.", "时间线已就绪；检查后可创建本地 MP4 预览。") : ui("There is no exportable timeline yet. This state will not start generation or a paid call.", "还没有可导出的时间线；此状态不会启动生成或付费调用。");
}
...
async function renderTutorial(id) {
  ...
  detail.innerHTML = `<button class="tutorial-back" type="button">← ${ui("All tutorials", "全部教程")}</button><p class="eyebrow">${escapeHtml(tutorial.category)} · ${escapeHtml(tutorial.level)}</p><h2>${escapeHtml(tutorial.title)}</h2><p class="tutorial-outcome">${escapeHtml(tutorial.outcome)}</p><p class="tutorial-project">${ui("Current project", "当前作品")}: ${escapeHtml(snapshot.project.name)}</p><div class="tutorial-meta"><span>${escapeHtml(tutorial.duration)}</span><span>${escapeHtml(tutorial.calls)}</span><span>${tutorial.nodes.length} ${ui("nodes", "个节点")}</span><span>v${tutorial.version}</span></div><p class="tutorial-explainer"><strong>${ui("This is a guide, not an automatic wizard.", "这是操作指南，不是自动向导。")}</strong> ${ui("Checking Complete only records your progress. It does not perform, verify, or unlock the step. Adding the blueprint creates editable nodes without generating media or spending allowance.", "勾选完成只记录进度，不会执行、验证或解锁步骤。添加蓝图只会创建可编辑节点，不会生成媒体或消耗额度。")}</p><p class="tutorial-readiness" role="status"><strong>${readiness ? ui("Media tools ready", "媒体工具可用") : ui("Limited mode", "能力受限")}</strong> · ${ui(`Image ${hasImage ? "ready" : "unavailable"}, video ${hasVideo ? "ready" : "unavailable"}. Planning is checked when you submit Create. This check does not call a model.`, `图片${hasImage ? "可用" : "不可用"}、视频${hasVideo ? "可用" : "不可用"}。规划能力会在提交创作时检查。此检查不会调用模型。`)}</p><div class="tutorial-actions"><button class="tutorial-start" type="button">${existingGroup ? ui("Open existing blueprint", "打开现有蓝图") : ui("1 · Add blueprint to this project", "1 · 把蓝图加入当前作品")}</button><button class="tutorial-create" type="button">${ui("2 · Continue in Create", "2 · 继续到创作工作台")}</button><label>${ui("Optional: upload a reference image", "可选：上传参考图")}<input class="tutorial-upload" type="file" accept="image/png,image/jpeg,image/webp"></label><button class="tutorial-assets" type="button">${ui("Open Asset library", "打开素材库")}</button></div><p class="tutorial-action-state" role="status">${ui("Start with button 1. Use button 2 after editing the Canvas examples.", "先点按钮 1；在画布修改示例后再点按钮 2。")}</p><h3>${ui("Steps", "操作步骤")}</h3><ol class="tutorial-steps">${tutorial.steps.map((step, index) => `<li><label><input type="checkbox" value="${index}"${progress.completedSteps.includes(index) ? " checked" : ""}><span><strong>${ui("Complete (records progress only)", "完成（只记录进度）")}</strong><br>${escapeHtml(step)}</span></label></li>`).join("")}</ol><p class="tutorial-progress" role="status">${progress.completedCount}/${progress.totalSteps} ${ui("steps complete", "步完成")} · ${statusLabel}</p><h3>${ui("Acceptance checklist", "验收清单")}</h3><ul>${tutorial.checks.map(check => `<li>${escapeHtml(check)}</li>`).join("")}</ul><section class="tutorial-shortcut"><h3>${ui("Advanced reviewed shortcut", "高级经审查快捷模板")}</h3><p>${ui("Optional advanced tool. Beginners can skip this section.", "这是可选高级工具，新手可以跳过。")}</p><label>${ui("Template", "模板")}<select>${shortcuts.map(item => `<option value="${escapeHtml(item.id)}">${escapeHtml(item.localizedLabel)}</option>`).join("")}</select></label><label>${ui("Reference image", "参考图")}<select class="tutorial-reference"><option value="">${ui("Choose an uploaded reference image", "选择已上传参考图")}</option>${references.map(asset => `<option value="${escapeHtml(asset.id)}">${escapeHtml(asset.filename)}</option>`).join("")}</select></label><label>${ui("Creative direction", "创作要求")}<textarea rows="3"></textarea></label><button type="button"${references.length ? "" : " disabled"}>${ui("Create reviewable node", "创建待审阅节点")}</button><p role="status">${references.length ? ui("No generation or cost occurs automatically.", "不会自动生成或产生费用。") : ui("Upload an image reference first.", "请先上传一张图片参考素材。")}</p></section>`;
  ...
  detail.querySelector(".tutorial-upload").onchange = async event => { const file = event.currentTarget.files?.[0], state = detail.querySelector(".tutorial-action-state"); if (!file) return; state.textContent = ui(`Uploading ${file.name}…`, `正在上传 ${file.name}…`); try { const asset = await uploadReference(file); selectedWorkbenchAsset = asset.id; await refresh(); state.textContent = ui(`Uploaded and selected ${asset.filename}. Continue in Create to bind it to all product shots.`, `已上传并选择 ${asset.filename}。继续到创作工作台后可绑定到全部产品镜头。`); } catch (error) { state.textContent = `${ui("Upload failed", "上传失败")}: ${error.message}`; } finally { event.currentTarget.value = ""; } };
}
...
$("#bind-workbench-asset-all").onclick = async event => { const button = event.currentTarget; if (!selectedWorkbenchAsset || !snapshot.storyboard?.shots?.length) return; button.disabled = true; $("#workbench-asset-state").textContent = ui("Binding the selected asset to every shot…", "正在把选中素材绑定到所有镜头…"); try { const shots = snapshot.storyboard.shots.map(shot => ({ ...shot, referenceAssetIds: [...new Set([...(shot.referenceAssetIds || []), selectedWorkbenchAsset])] })), storyboard = await api(`/api/v1/projects/${projectId}/storyboard`, json("PUT", { version: snapshot.storyboard.version, shots })); await refresh(); renderScriptStoryboard(snapshot.story, storyboard); $("#workbench-asset-state").textContent = ui(`Asset bound to all ${shots.length} shots.`, `素材已绑定到全部 ${shots.length} 个镜头。`); } catch (error) { $("#workbench-asset-state").textContent = `${ui("Binding failed", "绑定失败")}: ${error.message}`; } finally { button.disabled = !selectedWorkbenchAsset || !snapshot.storyboard?.shots?.length; } };
