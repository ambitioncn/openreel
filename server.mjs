...
export function createOpenReelServer(store = createMemoryStore(), platform = createPlatform(), { production = false, secureCookies = production, enforceCommercialPolicySafety = production, jsonLimit = 1024 * 1024, rateLimit = {}, logger = () => {}, readiness = () => ({ database: "ok", assets: "ok" }), metrics = createMetrics(), feedbackStore = createProductFeedbackStore(), adminToken = null, adminPasswordHash = null, arkService = null, workbenchPlanningModel = null, qwenTtsService = null, authorizeQwenTts = () => false, productUrlFetcher = createProductUrlFetcher(), publishingOrchestrator = null, publishingAccounts = () => [], publishingOAuth = null, commercialLifecycle = null, commercialJobs = null, commercialEstimate = null, agentFilmStore = createMemoryAgentFilmStore(), directorSystemStore = createDirectorSystemStore(), capabilityBaselineNow = () => new Date() } = {}) {
  ...
  return createServer(async (req, res) => {
    const requestId = correlationId(req.headers["x-request-id"]), requestStartedAt = Date.now(); res.setHeader("x-request-id", requestId); metrics.begin();
    res.once("finish", () => { metrics.end(res.statusCode, Date.now() - requestStartedAt); log("http_request", { requestId, method: req.method, path: new URL(req.url, "http://localhost").pathname, status: res.statusCode, authorization: req.headers.authorization, cookie: req.headers.cookie }); });
    try {
      ...
      else if (req.method === "POST" && api[0] === "auth" && api[1] === "register") { const account = platform.register(await input()); store.createUser({ id: account.id, name: account.email }); metrics.funnel?.("registration", "success"); out = account; }
      else if (req.method === "POST" && api[0] === "auth" && api[1] === "login") { const session = platform.login(await input()), csrf = randomBytes(24).toString("base64url"); metrics.funnel?.("login", "success"); return json(res, 200, { account: platform.authenticate(session.token), csrfToken: csrf, expiresAt: session.expiresAt }, { "set-cookie": [sessionCookie("openreel_session", session.token, session, secureCookies), sessionCookie("openreel_csrf", csrf, session, secureCookies)] }); }
      else if (req.method === "POST" && api[0] === "feedback" && api.length === 1) { out = feedbackStore.record(who, await input()); metrics.feedback?.(out.category, out.rating); }
      ...
      else if (req.method === "GET" && api[0] === "projects" && api[2] === "exports" && api[3] === "manifest") { const state = store.snapshot(api[1], who), commercialRender = state.assets.find(asset => asset.role === "render" && asset.metadata?.identity?.schema === "openreel-revision-identity/v1"); if (commercialRender) requireCommercialRelease({ commercialJobs, ownerId: who, projectId: api[1], revisionIdentity: commercialRender.metadata.identity }); out = store.exportManifest(api[1], who); metrics.funnel?.("export_ready", "success"); }
...
if (process.argv[1] && realpathSync(process.argv[1]) === realpathSync(fileURLToPath(import.meta.url))) {
  ...
  if (arkService && qwenTtsService && audioMaxCostCny > 0 && imageModel && videoModels.fast && videoModels.quality) {
    ...
    const masterImageInspector = createCommercialMasterImageInspector({ arkService, model: process.env.OPENREEL_COMMERCIAL_EVALUATION_MODEL, maximumCostCnyPerCall: evaluationMaxCostCnyPerCall, cnyPerUsd });
    const commercialOrchestrator = createCommercialProviderOrchestrator({ arkService, qwenTtsService, musicService, qualityInspector, masterImageInspector, audioMaxCostCny, audioSettledCny: 0, cnyPerUsd, maxPolls: Number(process.env.OPENREEL_COMMERCIAL_MAX_POLLS || 360), wait: () => new Promise(resolve => setTimeout(resolve, 2_000)), artifactSink: (principal, artifact) => databaseStore.retainCommercialArtifact(artifact.projectId, artifact, principal.accountId), referenceArtifactSource: (principal, request) => request.assetIds.map(assetId => { const content = databaseStore.assetContent(request.projectId, assetId, undefined, principal.accountId); return { ...content.manifest, bytes: content.bytes }; }), preservedArtifactSource: (principal, request) => request.assetIds.map(assetId => databaseStore.assetContent(request.projectId, assetId, undefined, principal.accountId).manifest), productionArtifactSource: (principal, request) => request.assetIds.map(assetId => databaseStore.assetContent(request.projectId, assetId, undefined, principal.accountId).manifest), compositionSink: (principal, composition) => databaseStore.composeCommercialProject(composition.projectId, composition, principal.accountId) });
  ...
  const server = createOpenReelServer(databaseStore, platform, { production, secureCookies: production, readiness: backend.health, logger, feedbackStore, adminToken: process.env.OPENREEL_ADMIN_KEY || null, adminPasswordHash: process.env.OPENREEL_ADMIN_PASSWORD_HASH || null, arkService, workbenchPlanningModel: process.env.OPENREEL_WORKBENCH_TEXT_MODEL || null, qwenTtsService, commercialLifecycle, commercialJobs: commercialJobStore, commercialEstimate, agentFilmStore, publishingOAuth, publishingOrchestrator, publishingAccounts: publishingOAuth ? input => publishingOAuth.accounts(input) : () => [] });
