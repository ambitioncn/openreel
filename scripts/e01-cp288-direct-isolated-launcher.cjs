...
const codeRoot = process.env.OPENREEL_CP288_CODE_ROOT;
if (!codeRoot || !fs.existsSync(path.join(codeRoot, "server.mjs"))) throw new Error("OPENREEL_CP288_CODE_ROOT is required");
const stateRoot = fs.mkdtempSync(path.join(os.tmpdir(), "openreel-cp288-state."));
...
Object.assign(env, {
  ...
  OPENREEL_E01_CP288_EXECUTE: "true", OPENREEL_E01_MAX_SUBMISSIONS: "1", OPENREEL_E01_MAX_UNITS: "6945"
});
...
async function cleanup(code) {
  ...
  fs.rmSync("/tmp/openreel-cp288-runner.mjs", { force: true });
  process.exit(code);
}
...
(async () => {
  ...
  process.stdout.write(JSON.stringify({ status: "preflight_passed", route: "direct_plan_v3", maximumSubmissions: 1, automaticRetries: 0, maximumUnits: 6_945 }) + "\n");
  ...
  runner = spawn(process.execPath, ["/tmp/openreel-cp288-runner.mjs"], { cwd: codeRoot, env, stdio: "inherit" });
