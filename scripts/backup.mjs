...
const checkDatabase = path => {
  ...
  try {
    ...
    for (const asset of entities.get("assets").values()) {
      ...
      const commercialResult = asset.role === "result" && ["openreel-commercial-artifact/v1", "openreel-commercial-production-asset/v1"].includes(asset.metadata?.schema);
      const retainedCanvasResult = asset.role === "result" && asset.metadata?.retainedEvidenceId && node?.assetId === asset.id;
      if (asset.role === "result" && !commercialResult && !retainedCanvasResult && (!job || job.assetId !== asset.id || asset.nodeId == null)) throw new Error("restored database job lifecycle relationship is invalid: result asset");
    }
...
const PUBLISHING_FILES = Object.freeze(["batches.json", "douyin-jobs.json", "instagram_reels-jobs.json", "oauth.json", "tiktok-jobs.json", "youtube_shorts-jobs.json"]);
const checkPublishingState = root => {
  if (!lstatSync(root).isDirectory() || lstatSync(root).isSymbolicLink()) throw new Error("publishing state root must be a regular directory");
  const actual = files(root).map(path => path.slice(resolve(root).length + 1));
  if (JSON.stringify(actual) !== JSON.stringify(PUBLISHING_FILES)) throw new Error("publishing state file set is incomplete or unexpected");
  for (const name of PUBLISHING_FILES) regularFile(join(root, name));
  const parse = name => { try { return JSON.parse(readFileSync(join(root, name), "utf8")); } catch { throw new Error(`publishing state is invalid JSON: ${name}`); } };
  const oauth = parse("oauth.json"), batches = parse("batches.json");
  if (oauth?.schema !== "openreel-publishing-oauth/v1" || !Array.isArray(oauth.pending) || !Array.isArray(oauth.bindings)) throw new Error("publishing OAuth state schema is invalid");
  if (batches?.schema !== "openreel-publishing-batch-store/v1" || !Array.isArray(batches.batches)) throw new Error("publishing batch state schema is invalid");
  for (const name of PUBLISHING_FILES.filter(name => name.endsWith("-jobs.json"))) { const state = parse(name); if (state?.schema !== "openreel-publishing-job-store/v1" || !Array.isArray(state.jobs)) throw new Error(`publishing job state schema is invalid: ${name}`); }
  return { files: PUBLISHING_FILES.length };
};

export async function createBackup({ database, assets, platform, publishing = null, destination }) {
  ...
  if (publishing !== null) { if (!existsSync(publishing)) throw new Error(`backup source missing: ${publishing}`); checkPublishingState(publishing); }
  ...
  try {
    ...
    if (publishing !== null) { mkdirSync(join(staging, "publishing"), { mode: 0o700 }); for (const name of PUBLISHING_FILES) copyStableFile(join(publishing, name), join(staging, "publishing", name)); checkPublishingState(join(staging, "publishing")); }
...
export function verifyBackup({ source, destination }) {
  ...
  if (expected.some(path => path !== "openreel.sqlite" && path !== "platform.json" && !path.startsWith("assets/") && !path.startsWith("publishing/"))) throw new Error("backup manifest file namespace is invalid");
  const publishingEntries = expected.filter(path => path.startsWith("publishing/")).map(path => path.slice("publishing/".length));
  if (publishingEntries.length && JSON.stringify(publishingEntries) !== JSON.stringify(PUBLISHING_FILES)) throw new Error("backup manifest publishing state is incomplete");
  ...
  try {
    ...
    if (publishingEntries.length) { cpSync(join(sourceRoot, "publishing"), join(staging, "publishing"), { recursive: true }); checkPublishingState(join(staging, "publishing")); }
    const databaseState = checkDatabase(join(staging, "openreel.sqlite"));
  ...
  return { database: "ok", assets: manifest.files.filter(item => item.path.startsWith("assets/")).length, publishing: publishingEntries.length ? "ok" : "absent", isolatedDestination: resolve(destination) };
}

if (process.argv[1] && basename(process.argv[1]) === "backup.mjs") {
  const [command, source, assets, platform, destination, publishing] = process.argv.slice(2);
  if (command === "create") console.log(JSON.stringify(await createBackup({ database: source, assets, platform, destination, publishing: publishing || null })));
  else if (command === "verify") console.log(JSON.stringify(verifyBackup({ source, destination: assets })));
  else { console.error("usage: backup.mjs create <database> <assets> <platform> <destination> [publishing-state] | verify <backup> <isolated-restore>"); process.exitCode = 2; }
}
